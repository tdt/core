...
class RdfParser extends Object {
    ...
    private function _is_forbidden_rdf_property_attribute($local_name) {
     ...
     * @access	private
     */
    private function _is_forbidden_rdf_property_element($local_name) {
        return ( $local_name == RDF_RDF )
                || ( $local_name == RDF_DESCRIPTION)
                || ( $local_name == RDF_ID)
                || ( $local_name == RDF_ABOUT )
     ...
     * @access	private
     */
    private function _is_rdf_node_element($local_name) {
     ...
     * @access	private
     */
    private function _is_forbidden_rdf_node_element($local_name) {
        return ( $local_name == RDF_RDF )
                || ( $local_name == RDF_ID)
                || ( $local_name == RDF_ABOUT )
                || ( $local_name == RDF_BAG_ID )
     ...
     * @access	private
     */
    private function _parse_uri($uri, $buffer, &$scheme, &$authority, &$path, &$query, &$fragment) {
    ...
    private function _resolve_uri_reference($base_uri, $reference_uri, &$buffer) {
        ...
        $this->_parse_uri($reference_uri, $reference_buffer, $reference_scheme, $reference_authority, $reference_path, $reference_query, $reference_fragment);

        $this->_parse_uri($base_uri, $base_buffer, $base_scheme, $base_authority, $base_path, $base_query, $base_fragment);
     ...
     * @access	private
     */
    private function _resolve_id($id, &$buffer) {
        $id_buffer = '';
     ...
     * @access	private
     */
    private function _split_name($name, &$buffer, &$namespace_uri, &$local_name) {
        static $nul = 0;
        $buffer = $name;
     ...
     * @access	private
     */
    private function _generate_anonymous_uri(&$buf) {
        $id = '';
     ...
     * @access	private
     */
    private function _report_statement($subject_type, $subject, $predicate, $ordinal, $object_type, $object, $xml_lang, $bag_id, $statements, $statement_id, $datatype) {
        ...
        if ($bag_id) {
            if ($statements == '') {
                $this->_report_statement(RDF_SUBJECT_TYPE_URI, $bag_id, RDF_NAMESPACE_URI . RDF_TYPE, 0, RDF_OBJECT_TYPE_RESOURCE, RDF_NAMESPACE_URI . RDF_BAG, '', '', '', '', $datatype);
            }
            ...
            $this->_report_statement(
                    RDF_SUBJECT_TYPE_URI, $bag_id, $predicate_buffer, $statements, RDF_OBJECT_TYPE_BNODE, $statement_id, '', '', '', '', $datatype);
        }

        if ($statement_id) {
            // rdf:type = rdf:Statement
            $this->_report_statement(
                    $statement_id_type, $statement_id, RDF_NAMESPACE_URI . RDF_TYPE, 0, RDF_OBJECT_TYPE_RESOURCE, RDF_NAMESPACE_URI . RDF_STATEMENT, '', '', '', '', $datatype);
            ...
            // rdf:subject
            $this->_report_statement(
                    $statement_id_type, $statement_id, RDF_NAMESPACE_URI . RDF_SUBJECT, 0, $obj_type, $subject, '', '', '', '', $datatype);

            // rdf:predicate
            $this->_report_statement(
                    $statement_id_type, $statement_id, RDF_NAMESPACE_URI . RDF_PREDICATE, 0, RDF_OBJECT_TYPE_RESOURCE, $predicate, '', '', '', '', $datatype);

            // rdf:object
            $this->_report_statement(
                    $statement_id_type, $statement_id, RDF_NAMESPACE_URI . RDF_OBJECT, 0, $object_type, $object, '', '', '', '', $datatype);
        }
    }
    ...
    private function _handle_property_attributes($subject_type, $subject, $attributes, $xml_lang, $bag_id, $statements) {
        ...
        for ($i = 0; isset($attributes[$i]); $i += 2) {
            $this->_split_name(
                    $attributes[$i], $attribute, $attribute_namespace_uri, $attribute_local_name);
            ...
            if (RDF_NAMESPACE_URI == $attribute_namespace_uri) {
                if ($this->_is_rdf_property_attribute_literal($attribute_local_name)) {
                    $this->_report_statement(
                            $subject_type, $subject, $predicate, 0, RDF_OBJECT_TYPE_LITERAL, $attribute_value, $xml_lang, $bag_id, $statements, '', '');
                } else if ($this->_is_rdf_property_attribute_resource($attribute_local_name)) {
                    $this->_report_statement(
                            $subject_type, $subject, $predicate, 0, RDF_OBJECT_TYPE_RESOURCE, $attribute_value, '', $bag_id, $statements, '', '');
                } else if (( $ordinal = $this->_is_rdf_ordinal($attribute_local_name) ) != 0) {
                    $this->_report_statement(
                            $subject_type, $subject, $predicate, $ordinal, RDF_OBJECT_TYPE_LITERAL, $attribute_value, $xml_lang, $bag_id, $statements, '', '');
    ...
    private function _report_error($error) {
    ...
    private function _handle_resource_element($namespace_uri, $local_name, $attributes, $parent) {
        ...
        if (($local_name != RDF_DESCRIPTION )
            ...
            $this->_report_statement(
                    $this->rdf_parser['top']['subject_type'], $this->rdf_parser['top']['subject'], RDF_NAMESPACE_URI . RDF_TYPE, 0, RDF_OBJECT_TYPE_RESOURCE, $type, '', $this->rdf_parser['top']['bag_id'], $this->rdf_parser['top']['statements'], '', $datatype);
        }
        ...
        if ($parent) {
            ...
            $this->_report_statement(
                    $parent['parent']['subject_type'], $parent['parent']['subject'], $parent['predicate'], $parent['ordinal'], $objtype, $this->rdf_parser['top']['subject'], '', $parent['parent']['bag_id'], $parent['parent']['statements'], $parent['statement_id'], $parent['datatype']);
        }

        if ($this->rdf_parser['top']['has_property_attributes']) {
            $this->_handle_property_attributes(
                    $this->rdf_parser['top']['subject_type'], $this->rdf_parser['top']['subject'], $attributes, $this->rdf_parser['top']['xml_lang'], $this->rdf_parser['top']['bag_id'], $this->rdf_parser['top']['statements']);
        }
    }
    ...
    private function _handle_property_element(&$namespace_uri, &$local_name, &$attributes) {
        ...
        for ($i = 0; isset($attributes[$i]); $i += 2) {
            $this->_split_name(
                    $attributes[$i], $buffer, $attribute_namespace_uri, $attribute_local_name);
        ...
        if ($node_id) {
            ...
            if ($statement_id) {
                // reify statement
                $this->_report_statement(
                        $this->rdf_parser['top']['parent']['subject_type'], $this->rdf_parser['top']['parent']['subject'], $this->rdf_parser['top']['predicate'], $this->rdf_parser['top']['ordinal'], RDF_OBJECT_TYPE_BNODE, $node_id, '', $this->rdf_parser['top']['parent']['bag_id'], $this->rdf_parser['top']['parent']['statements'], $this->rdf_parser['top']['statement_id'], '');
                $statement_id = '';
            } else {
                $this->_report_statement(
                        $this->rdf_parser['top']['parent']['subject_type'], $this->rdf_parser['top']['parent']['subject'], $this->rdf_parser['top']['predicate'], $this->rdf_parser['top']['ordinal'], RDF_OBJECT_TYPE_BNODE, $node_id, '', $this->rdf_parser['top']['parent']['bag_id'], $this->rdf_parser['top']['parent']['statements'], '', $datatype);
            }
        ...
        if ($parse_type) {
            ...
            if ($attribute_value == RDF_PARSE_TYPE_RESOURCE) {
                ...
                $this->_report_statement(
                        $this->rdf_parser['top']['parent']['subject_type'], $this->rdf_parser['top']['parent']['subject'], $this->rdf_parser['top']['predicate'], 0, RDF_OBJECT_TYPE_BNODE, $buffer, '', $this->rdf_parser['top']['parent']['bag_id'], $this->rdf_parser['top']['parent']['statements'], $this->rdf_parser['top']['statement_id'], $datatype);
                ...
                $this->_report_statement(
                        $this->rdf_parser['top']['parent']['subject_type'], $this->rdf_parser['top']['parent']['subject'], $this->rdf_parser['top']['predicate'], 0, RDF_OBJECT_TYPE_XML, '', '', $this->rdf_parser['top']['parent']['bag_id'], $this->rdf_parser['top']['parent']['statements'], $this->rdf_parser['top']['statement_id'], $datatype);
            ...
            // since we are sure that this is now a resource property we can report it.
            $this->_report_statement(
                    $this->rdf_parser['top']['parent']['subject_type'], $this->rdf_parser['top']['parent']['subject'], $this->rdf_parser['top']['predicate'], $this->rdf_parser['top']['ordinal'], $object_type, $buffer, '', $this->rdf_parser['top']['parent']['bag_id'], $this->rdf_parser['top']['parent']['statements'], $this->rdf_parser['top']['statement_id'], $datatype); // should we allow IDs?
            ...
            if ($this->rdf_parser['top']['has_property_attributes']) {
                $this->_handle_property_attributes(
                        $subject_type, $buffer, $attributes, $this->rdf_parser['top']['xml_lang'], $this->rdf_parser['top']['bag_id'], $this->rdf_parser['top']['statements']);
            }
        }
    }
     ...
     * @access	private
     */
    private function _handle_collection_element(&$namespace_uri, &$local_name, &$attributes) {
        $aux2 = Array();
        foreach ($attributes as $atkey => $atvalue) {
            $aux2[] = $atkey;
            $aux2[] = $atvalue;
        ...
        for ($i = 0; isset($attributes[$i]); $i += 2) {
            ...
            if ($attribute_namespace_uri == '' || $attribute_namespace_uri == RDF_NAMESPACE_URI) {
                ...
                if (!( ($namespace_uri == RDF_NAMESPACE_URI || $namespace_uri == '')
                        && ($local_name == RDF_DESCRIPTION || $local_name == RDF_LI) )) {
                    $this->_report_statement(
                            $tmp_subject_type, $id_buffer, RDF_NAMESPACE_URI . RDF_TYPE, '', RDF_OBJECT_TYPE_RESOURCE, $namespace_uri . $local_name, '', '', '', '', '');
                }
            }
        }
...
// collection construction site
     ...
     * @access	private
     */
    private function _handle_xml_start_element(&$namespace_uri, &$local_name, &$attributes) {
        $aux2 = Array();
        foreach ($attributes as $atkey => $atvalue) {
            $aux2[] = $atkey;
            $aux2[] = $atvalue;
     ...
     * @access	private
     */
    private function _handle_xml_end_element($name) {
        $buffer = '';
        $namespace_uri = '';
        $local_name = '';

        $this->_split_name(
                $name, $buffer, $namespace_uri, $local_name);
     ...
     * @access	private
     */
    private function _join_name_and_declare_prefix($namespace_uri, $local_name) {
     ...
     * @access	private
     */
    private function _end_collection() {
        ...
        if (isset($this->rdf_parser['top']['collection'])) {
            ...
            for ($i = 0; isset($this->rdf_parser['top']['collection']['object_label'][$i]); $i++) {
                ...
                $this->_report_statement(
                        RDF_SUBJECT_TYPE_BNODE, $subject, RDF_NAMESPACE_URI . RDF_FIRST, '', $this->rdf_parser['top']['collection']['object_type'][$i], $this->rdf_parser['top']['collection']['object_label'][$i], '', '', '', '', '');
                ...
                $this->_report_statement(
                        RDF_SUBJECT_TYPE_BNODE, $subject, RDF_NAMESPACE_URI . RDF_REST, '', $obj_type_2, $object_2, '', '', '', '', '');
        ...
        switch ($this->rdf_parser['top']['state']) {
            ...
            case IN_PROPERTY_UNKNOWN_OBJECT:
                ...
                $this->_handle_resource_element(
                        $namespace_uri, $local_name, $attributes, $this->rdf_parser['top']['parent']);
                break;
    ...
    private function _end_literal_property() {
        ...
        $this->_report_statement(
                $this->rdf_parser['top']['parent']['subject_type'], $this->rdf_parser['top']['parent']['subject'], $this->rdf_parser['top']['predicate'], $this->rdf_parser['top']['ordinal'], RDF_OBJECT_TYPE_LITERAL, $this->rdf_parser['top']['data'], $this->rdf_parser['top']['xml_lang'], $this->rdf_parser['top']['parent']['bag_id'], $this->rdf_parser['top']['parent']['statements'], $this->rdf_parser['top']['statement_id'], $this->rdf_parser['top']['datatype']);
    }
...
//			= str_replace($search, $replace, $this->rdf_parser["xml_literal"]["buffer"]);
     ...
     */
    private function add_statement_to_model(
    &$user_data, $subject_type, $subject, $predicate, $ordinal, $object_type, $object, $xml_lang, $datatype) {
     ...
     * @return	object MemModel
     */
    public function generateModel($base, $rdfBaseURI = false, $model = false) {
     ...
     * @access	private
     */
    private function rdf_parser_create($encoding) {
     ...
     * @access	private
     */
    private function _start_ns_declaration_handler(&$parser, $ns_prefix, $ns_uri) {
        if (!$ns_prefix)
     ...
     * @access	private
     */
    private function rdf_parser_free() {
        $z = 3;
     ...
     * @access	private
     */
    private function rdf_parse($s, $is_final) {
     ...
     * @access	private
     */
    private function rdf_get_xml_parser() {
     ...
     * @access	private
     */
    private function rdf_set_base($base) {
