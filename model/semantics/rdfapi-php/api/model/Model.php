 ...
 * @author Miel Vander Sande <miel.vandersande@ugent.be>
 *
 * @package model
 * @access	public
 */
class Model extends Object {
        ...
        if (is_a($subject, "BlankNode")) {
            ...
            if (!array_key_exists($label, $blankNodes_tmp)) {
                if ($this->findFirstMatchingStatement($subject, NULL, NULL)
                    ...
                    $statement->setSubject($blankNodes_tmp[$label]);
                } else {
        ...
        if (is_a($object, "BlankNode")) {
            ...
            if (!array_key_exists($label, $blankNodes_tmp)) {
                if ($this->findFirstMatchingStatement($object, NULL, NULL)
                    ...
                    $blankNodes_tmp[$label] = new BlankNode($this);
                    $statement->setObject($blankNodes_tmp[$label]);
                } else {
     ...
     * @access	public
     */
    public function getOntModel($vocabulary) {
     ...
     *
     */
    public function getMemModelByRDQL($queryString, $closure = FALSE) {
...
// and add them to the result, if closure = true
     ...
     * @throws   PhpError
     */
    public function visualize($format = "dot", $short_prefix = TRUE) {
     ...
     * @return string/array
     */
    public function sparqlQuery($query, $resultform = false) {
        list($engine, $dataset) = $this->_prepareSparql();
        return $engine->queryModel(
                        $dataset, $this->_parseSparqlQuery($query), $resultform
        );
    }
...
//function sparqlQuery($query,$resultform = false)
     ...
     */
    public function sparqlPrepare($query) {
        list($engine, $dataset) = $this->_prepareSparql();
        return $engine->prepare(
                        $dataset, $this->_parseSparqlQuery($query)
        );
    }
...
//function sparqlPrepare($query)
     ...
     */
    private function _prepareSparql() {
...
//function _prepareSparql()
     ...
     */
    private function _parseSparqlQuery($query) {
