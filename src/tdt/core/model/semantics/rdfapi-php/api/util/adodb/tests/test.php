...
if (defined('CHECKWSFAIL')){ echo " TESTING $conn ";
flush();
}
...
if ($rs->fields['id'] != 1) {Err("Error");
print_r($rs->fields);
};
if (trim($rs->fields['firstname']) != 'Caroline') {print Err("Error 2");
print_r($rs->fields);
};

$rs->MoveNext();
if ($rs->fields['id'] != 2) {Err("Error 3");
print_r($rs->fields);
};
$rs->MoveNext();
if (!$rs->EOF) {
...
if ($rs->fields[$id] != 1) {Err("Error 1");
print_r($rs->fields);
};
if (trim($rs->fields[$fname]) != 'Caroline') {Err("Error 2");
print_r($rs->fields);
};
$rs->MoveNext();
if ($rs->fields[$id] != 2) {Err("Error 3");
...
if (trim($rs->fields[1]) != 'Caroline') {Err("Error 2");
print_r($rs->fields);
};
$rs->MoveNext();
if (!$rs->EOF) Err("Error EOF");
...
<?php
	...
	printf("<p>Total queries = %d;
total cached = %d</p>",$EXECS+$CACHED, $CACHED);
	flush();
}
...
global $TESTERRS,$ERRNO;
	...
	$ERRNO = $errno;
	$TESTERRS += 1;
	print "<i>** $dbms ($fn): errno = $errno &nbsp;
errmsg = $errmsg ($p1, $p2)</i><br>";
}
